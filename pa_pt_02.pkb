create or replace PACKAGE BODY pa_pt_02 IS
   g_start    NUMBER;
   g_end      NUMBER;
   g_time01   NUMBER;
   g_time02   NUMBER;
   ...
   BEGIN
      ...
      g_time01 := NVL(g_time01, 0) + (g_end - g_start) / 100;
   ...
   BEGIN
      ...
      WHERE id_no = pi_id_no;
      g_end := DBMS_UTILITY.GET_TIME;
      g_time02 := NVL(g_time02, 0) + (g_end - g_start) / 100;
      ...
      /*FOR rc IN navigate_cr LOOP
         ...
         g_end := DBMS_UTILITY.GET_TIME;
         g_time03 := NVL(g_time03, 0) + (g_end - g_start) / 100;
         --
         IF l_working1 IS NOT NULL THEN
            ...
            g_end := DBMS_UTILITY.GET_TIME;
            g_time04 := NVL(g_time04, 0) + (g_end - g_start) / 100;
         END IF;
      END LOOP;*/
      g_start := DBMS_UTILITY.GET_TIME;
      FOR rc IN ma_cr LOOP
         IF rc.seq_no > pi_ma THEN
            UPDATE pt_analysis3 SET l_idx_avg = rc.l_idx_avg, l_idx_stdev = rc.l_idx_stddev WHERE CURRENT OF ma_cr;
         END IF;
      END LOOP;
      g_end := DBMS_UTILITY.GET_TIME;
      g_time03 := NVL(g_time03, 0) + (g_end - g_start) / 100;
      ...
      UPDATE pt_analysis3
          ...
          AND seq_no > pi_ma;
      g_end := DBMS_UTILITY.GET_TIME;
      g_time05 := NVL(g_time05, 0) + (g_end - g_start) / 100;
      -- Determine signal
      g_start := DBMS_UTILITY.GET_TIME;
      UPDATE pt_analysis3
      ...
      g_end := DBMS_UTILITY.GET_TIME;
      g_time06 := NVL(g_time06, 0) + (g_end - g_start) / 100;
      -- Buy/Sell
      FOR rc IN navigate_cr LOOP
         ...
         IF NVL(l_tmp_rec.sts, 'CLOSED') = 'CLOSED' THEN -- No opened position
            IF rc.signal = 'BUY' THEN
               ...
               g_end := DBMS_UTILITY.GET_TIME;
               g_time07 := NVL(g_time07, 0) + (g_end - g_start) / 100;
         ...
         IF l_tmp_rec.sts IN ('BOUGHT','SOLD','CLOSED') OR rc.signal IN ('BUY','SELL') THEN
            ...
            UPDATE pt_analysis3
               ...
               WHERE CURRENT OF navigate_cr;
            g_end := DBMS_UTILITY.GET_TIME;
            g_time08 := NVL(g_time08, 0) + (g_end - g_start) / 100;
         END IF;
      END LOOP;
      ...
      FOR rc IN navigate_cr LOOP
         ...
         UPDATE pt_analysis3
            ...
            WHERE CURRENT OF navigate_cr;
         g_end := DBMS_UTILITY.GET_TIME;
         g_time09 := NVL(g_time09, 0) + (g_end - g_start) / 100;
      END LOOP;
      RETURN 0;
   END back_test;
            ...
            FOR pl_ratio IN pi_profit_loss_ratio_fm..pi_profit_loss_ratio_to LOOP
               ...
               BEGIN
                  ...
                  INSERT INTO pt_reslt
                     ...
                     WHERE id_no = l_id_no);
                  g_end := DBMS_UTILITY.GET_TIME;
                  g_time10 := NVL(g_time10, 0) + (g_end - g_start) / 100;
               EXCEPTION
   ...
   BEGIN
      ...
      DBMS_OUTPUT.PUT_LINE( 'g_time01 = ' || g_time01 );
      DBMS_OUTPUT.PUT_LINE( 'g_time02 = ' || g_time02 );
      DBMS_OUTPUT.PUT_LINE( 'g_time03 = ' || g_time03 );
      DBMS_OUTPUT.PUT_LINE( 'g_time04 = ' || g_time04 );
      DBMS_OUTPUT.PUT_LINE( 'g_time05 = ' || g_time05 );
      DBMS_OUTPUT.PUT_LINE( 'g_time06 = ' || g_time06 );
      DBMS_OUTPUT.PUT_LINE( 'g_time07 = ' || g_time07 );
      DBMS_OUTPUT.PUT_LINE( 'g_time08 = ' || g_time08 );
      DBMS_OUTPUT.PUT_LINE( 'g_time09 = ' || g_time09 );
      DBMS_OUTPUT.PUT_LINE( 'g_time10 = ' || g_time10 );
   END run_back_test_set;
END pa_pt_02;
/

create or replace PACKAGE BODY pa_pt IS
   FUNCTION merge_price(pi_ticker_a IN VARCHAR2, pi_ticker_b IN VARCHAR2) RETURN NUMBER IS
   BEGIN
      RETURN pa_pt_01.merge_price( pi_ticker_a, pi_ticker_b );
   END merge_price;
   --
   ...
   BEGIN
      RETURN pa_pt_01.back_test(pi_id_no
                        , pi_ma
                        , pi_buy_threshold
                        , pi_sell_threshold
                        , pi_lot_amt
   ...
   BEGIN
      pa_pt_01.run_back_test_set(pi_ticker_a
                     , pi_ticker_b
                     , pi_lot_amt
                     , pi_max_nof_lot
                     , pi_stop_loss_pct_fm
